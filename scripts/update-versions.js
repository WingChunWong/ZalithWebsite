...
async function main() {
	...
	const officialPublicDir = path.join(
		__dirname,
		"..",
		"official-website",
		"public",
	...
	try {
		...
		const zl1Release = await fetchLatestRelease(
		...
		if (hasChanges(oldZl1Data, zl1Data)) {
			...
			console.log(
				`✅ ZL1 version ${zl1Data.latest_version} is already up to date`,
			);
		}

		// 更新ZL2版本信息
		console.log("\n📦 Fetching ZL2 latest release...");
		const zl2Release = await fetchLatestRelease(
		...
		if (hasChanges(oldZl2Data, zl2Data)) {
			...
			console.log(
				`✅ ZL2 version ${zl2Data.latest_version} is already up to date`,
			);
		}
