...
async function fetchLemwoodData() {
	...
	try {
		...
		try {
			...
			const proxyResponse = await fetch(
				`https://api.allorigins.win/get?url=${encodeURIComponent(lemwoodUrl)}`,
				{
					headers: {
						Accept: "application/json",
					},
...
watch(
	[localizedDescription, latestRelease],
	async ([newDesc, newRelease]) => {
...
async function fetchLatestRelease() {
	...
	try {
		...
		hasError.value = true;
		errorMessage.value =
			(error as any).message || "无法获取版本信息，请检查网络连接或稍后重试";
	}
}
