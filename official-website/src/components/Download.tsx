...
const DownloadSection = () => {
		...
		<div className="glass-card p-4 sm:p-8 animate-pulse relative overflow-hidden">
			...
			<div className="space-y-3 relative z-10">
				{[1, 2, 3].map((i) => (
					<div
						key={i}
						className="h-20 bg-[var(--bg-alt)] rounded-2xl flex items-center px-4 gap-4"
					>
			...
			<div className="space-y-4 relative z-10 flex-1">
				{[1, 2, 3, 4, 5].map((i) => (
					<div key={i} className="space-y-2">
						<div
							className="h-4 bg-[var(--bg-alt)] rounded"
							style={{ width: `${80 - i * 10}%` }}
						/>
	...
	const currentDevice =
		dynamicDeviceTypes.find((d) => d.id === selectedDevice) ||
		dynamicDeviceTypes[0];
	const currentSource =
		downloadSources.find((s) => s.id === selectedSource) || downloadSources[0];
	...
	const getDownloadUrl = (asset: Asset) => {
		...
		if (selectedSource === "foxington" && Array.isArray(mirrorData.foxington)) {
			...
			const matched = mirrorData.foxington.find(
				(f: any) => f.name === targetArch,
			);
		...
		if (
			selectedSource === "haha" &&
			mirrorData.haha?.files &&
			Array.isArray(mirrorData.haha.files)
		) {
			const fileName = asset.name.toLowerCase();
			let targetArch = "";
			...
			const matched = mirrorData.haha.files.find(
				(f: any) =>
					f.arch === targetArch ||
					(!targetArch && (!f.arch || f.arch === "all")),
			);
		...
		if (selectedSource === "lemwood" && Array.isArray(mirrorData.lemwood)) {
			...
			let matchedRelease = mirrorData.lemwood.find(
				(r: any) =>
					r.tag_name === currentTagName || r.tag_name === normalizedTagName,
			);
			...
			for (let i = mirrorData.lemwood.length - 1; i >= 0; i--) {
				if (Array.isArray(mirrorData.lemwood[i].assets)) {
					const asset_match = mirrorData.lemwood[i].assets.find(
						(a: any) => a.name === asset.name,
					);
	...
	const filteredAssets =
		release?.assets.filter((asset) => {
			...
			return currentDevice.patterns.some(
				(p) => p === "*" || name.includes(p.toLowerCase()),
			);
		}) || [];
	...
	return (
		<section
			id="download"
			className="py-24 relative overflow-hidden bg-[var(--bg-alt)] transition-colors duration-300"
		>
									...
									<div className="glass-card p-4 sm:p-8">
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
											...
											<div className="relative">
												<label className="block text-xs font-bold text-[var(--text-2)] mb-2 uppercase tracking-wider">
													{t("download.deviceType")}
												</label>
												<button
																	...
																	>
																		<div className="flex flex-col">
																			<div className="flex items-center gap-2">
																				...
																				<span className="px-1.5 py-0.5 bg-green-500/10 text-green-600 dark:text-green-400 text-[10px] rounded uppercase">
																					{s.speed}
																				</span>
																			</div>
																			<span className="text-xs text-[var(--text-2)]">
													...
													<div className="flex items-center gap-3">
														...
														<div>
															<p className="text-sm font-bold text-[var(--text-1)]">
																{t("download.cloudDrive")}
															</p>
															<p className="text-xs text-[var(--text-2)]">
																{t("download.cloudDriveDesc")}
															</p>
														</div>
													</div>
													<a
													...
													>
														<div className="flex items-start gap-4 flex-1 w-full">
															...
															<div className="min-w-0 flex-1">
																<p className="text-sm font-bold text-[var(--text-1)] break-all sm:break-normal line-clamp-2 sm:line-clamp-1">
																	{asset.name}
																</p>
																<div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
																	<span className="text-[10px] text-[var(--text-2)] font-medium uppercase whitespace-nowrap">
