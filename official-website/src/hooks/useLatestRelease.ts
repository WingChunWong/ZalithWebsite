...
const DOWNLOAD_SOURCES: DownloadSource[] = [
	...
	},
	{
		id: "mirror",
		name: "国内加速",
		description: "fishcpy 提供",
		speed: "国内较快",
		contributor: { name: "fishcpy", url: "https://github.com/fishcpy" },
	},
	{
		id: "foxington",
		name: "Foxington 源",
...
export const useLatestRelease = (
	...
	const localVersionFile =
		project === "zl1" ? "/version.json" : "/version2.json";
		...
		const load = async () => {
				...
				try {
					...
					try {
						...
						if (isMounted) {
							...
							localStorage.setItem(
								`${cacheKeyPrefix}release`,
								JSON.stringify(fallbackRelease),
							);
							setIsReleaseLoading(false);
						}
					} catch (le) {
			...
			const fetchMirrorsTask = async () => {
				...
				if (isMounted) {
					...
					localStorage.setItem(
						`${cacheKeyPrefix}mirrors`,
						JSON.stringify(newMirrorData),
					);
					setIsMirrorsLoading(false);
				}
			};
			...
			const detectIPTask = async () => {
				try {
					...
					const isCN =
						tz.includes("Asia/Shanghai") || tz.includes("Asia/Chongqing");
					if (isMounted) setIsChinaIP(isCN);
				}
			};
	...
	const dynamicDeviceTypes = useMemo(() => {
		const baseTypes: DeviceType[] = [
			{
				id: "all",
				name: t("download.devices.all"),
				description: t("download.devices.allDesc"),
				patterns: ["*"],
			},
		];
		...
		release.assets.forEach((asset) => {
			...
			else if (name.includes("armeabi-v7a") || name.includes("armeabi"))
